...
ggsave("docs/figures/S1-sras.png", psrastrains, device = "png", dpi = 300, width = 9, height = 5)
## add endo data
...
```
![Contaminated SRA](blob_SRR2155541/blob_all/all.blobDB.json.bestsum.species.p8.span.100.blobplot.velvet.png)

### Identifying poor taxonomic assignments
In addition to those mentioned above with poor assemblies, we filtered out any SRAs with inconclusive domain-level agreement.
```{r}
...
ggplot(tax_summary, aes(x=domain_perc, fill=domain_perc>70)) + geom_histogram() + 
  labs(title="Taxonomic Assignment", subtitle="Histogram of Domain-level taxonomic assignment percentages", y="Number of sequences", fill="Domain-level Assignment > 70%") + theme(legend.position = "bottom") 
