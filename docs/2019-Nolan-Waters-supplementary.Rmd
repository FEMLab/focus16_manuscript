...
results_dir <- "./results/2019-11-06-results/"
```
...
silva_headers <- data.frame("raw"=names(silva), stringsAsFactors = F)
silva_headers$pre <- gsub("(\\D*).*", "\\1", silva_headers$raw)
silva_headers$id <- gsub("(.*?)\\.(.*)", "\\1", silva_headers$raw)
silva_headers$strain <- gsub("(.*?)\\.(.*?)\\.(.*)\\s.*;(.*)", "\\4", silva_headers$raw)
silva_headers$genus <- gsub("(.*?)\\s(.*?)[\\s.]*", "\\2", silva_headers$strain)
silva_headers$species <- gsub("(.*?)\\s(.*?)[\\s.*]", "\\2", silva_headers$strain)

#silva_headers$name <- gsub(">(.*?)\\s.*", "\\1", silva_headers$raw)
silva_headers$start <- as.numeric(gsub("(.*)\\.(.+)\\.(.*?)\\s.*", "\\2", silva_headers$raw))
silva_headers$stop <-  as.numeric(gsub("(.*)\\.(.+)\\.(.*?)\\s.*", "\\3", silva_headers$raw))
...
silva_headers$matcher <- ifelse(
  silva_headers$cg, 
  silva_headers$id,
  ifelse(silva_headers$wgs, 
         substr(silva_headers$id, start = 1, stop =( nchar(silva_headers$pre) + 2)),
         NA)
)
```
...
cd /mnt/shared/scratch/synology/nw42839/2019-11-02-focusdb/
...
```{bash eval=FALSE}
mkdir 2019-11-04-focusdb
while read genus; do echo $genus; cp ./${genus}_genus/${genus}_ribo16s.fasta ./2019-11-06-results/ ; cp ./${genus}_genus/focusDB.log ./2019-11-06-results/${genus}.log ; cp ./${genus}_genus/sequence_summary.tab  ./2019-11-06-results/${genus}_seq_summary.tab; done < ~/GitHub/focusdb_manuscript/docs/datasets/combined_genuses.txt
cat ./*_genus/SUMMARY > 2019-11-06-results/summary_all
tar czf 2019-11-06-results.tar.gz 2019-11-06-results/
...
all_focus <- readRNAStringSet("./results/all_focusDB_ribo16s.fasta")
...
###
focusdb_headers$pre <- gsub("(\\D*).*", "\\1", focusdb_headers$raw)
silva_headers$id <- gsub("(.*?)\\.(.*)", "\\1", silva_headers$raw)
silva_headers$strain <- gsub("(.*?)\\.(.*?)\\.(.*)\\s.*;(.*)", "\\4", silva_headers$raw)
silva_headers$genus <- gsub("(.*?)\\s(.*?)[\\s.]*", "\\2", silva_headers$strain)
silva_headers$species <- gsub("(.*?)\\s(.*?)[\\s.*]", "\\2", silva_headers$strain)
