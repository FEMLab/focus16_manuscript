...
1)  `sraFind` is used to identify whole-genome sequencing SRA accessions for the organism of interest.  All of these are downloaded with fasterq-dump[cite].
2) `plentyofbugs` is used to download all (or a user-defined subsample) of potential reference genomes from Refseq.  
The following process occurs in a loop for each SRA:
3)   Reads are run through several quality control steps. This  will reject SRAs where reads are too short to be effectivly used with riboSeed  (<65bp)The average length of the reads is checked based on the first 30 reads. This is to prevent further processing of an SRA that contains reads of very low length (<65bp). As riboSeed cannot assemble genomes with reads of this length.
...
rows <- seq(.1, .9, .125)  # 6 rows

shadowbox(box.type = "rect", mid = c(0.48, 0.55), box.col="gray95",lcol="gray85",
          rady = 0.25, radx = 0.3, angle = 0, shadow.size=0)
shadowbox(box.type = "rect", mid = c(0.49, 0.54),  box.col="gray95", lcol="gray85",
          rady = 0.25, radx = 0.3, angle = 0, shadow.size=0)
shadowbox(box.type = "rect", mid = c(0.5, 0.53), box.col="gray95", lcol="gray85",
          rady = 0.25, radx = 0.3, angle = 0, shadow.size=0)
# shadowbox(box.type = "rect", mid = c(0.52, 0.52),
#           rady = 0.4, radx = 0.2, angle = 0)
straightarrow(from =  c(cols[1], rows[7]),
              to = c(cols[3], rows[6]),
              arr.length=.2, arr.width=.2,
              lwd = 1, arr.pos = 0.6)
straightarrow(from =  c(cols[5], rows[7]),
              to = c(cols[3], rows[6]),
              arr.length=.2, arr.width=.2,
              lwd = 1, arr.pos = 0.65)
...
textellipse(c(cols[5], .95),
         ...
         shadow.col = "#7b6c60", shadow.size = 0.005, cex = cexd)
textrect(c(cols[1], rows[7]), 
         boxsize, lab = "Download Reference\nGenomes for Species",
         box.col = "#ffe4ca",rady=thisrady, radx=thisradx,
         shadow.col = "#7b6c60", shadow.size = 0.005, cex = cexd) 
textrect(c(cols[5], rows[7]), boxsize, lab = "Identify and Download\nSRAs for Species",
         box.col = "#ffe4ca",rady=thisrady, radx=thisradx, 
         shadow.col = "#7b6c60", shadow.size = 0.005, cex = cexd) 
textrect(c(cols[3], rows[6]), boxsize, lab = "For each SRA:",
         box.col = "white",rady=thisrady, radx=thisradx, 
         shadow.col = "grey80", shadow.size = 0.005, cex = cexd) 
##  Sra Stuff
textrect(c(cols[2], rows[5]), boxsize, lab = "QC: \ncheck read length:\nQuality Trim (sickle)",
         box.col = "white",rady=thisrady, radx=thisradx, 
         shadow.col = "grey80", shadow.size = 0.005, cex = cexd) 
textrect(c(cols[2], rows[4]), boxsize, lab = "Downsample if \nCoverage > 50x (seqtk)",
         box.col = "white",rady=thisrady, radx=thisradx, 
         shadow.col = "grey80", shadow.size = 0.005, cex = cexd) 
##  Reference Selection
...
#          box.col = "white",rady=thisrady, radx=thisradx, 
#          shadow.col = "grey80", shadow.size = 0.005, cex = cexd) 
textrect(c(cols[4], .55), boxsize, lab = "Select Best Reference\n(plentyofbugs)",
         box.col = "white",rady=thisrady, radx=thisradx, 
         shadow.col = "grey80", shadow.size = 0.005, cex = cexd) 
textrect(c(cols[3], rows[3]), boxsize, lab = "De fere novo Assembly\n(riboSeed)",
         box.col = "white",rady=thisrady, radx=thisradx, 
         shadow.col = "grey80", shadow.size = 0.005, cex = cexd) 
textrect(c(cols[3], rows[2]), boxsize, 
            lab = "Extract 16s from \n new assemblies (barrnap)",
            box.col = "#a0ff9e",rady=thisrady, radx=thisradx,
             shadow.col = "#83d283", shadow.size = 0.005, cex = cexd) 
## Finalize
textrect(c(cols[1], rows[2]), boxsize,  lab = "Extract 16S for \nSpecies from Silva",
         box.col = "#ffe4ca",rady=thisrady, radx=thisradx, 
         shadow.col = "#7b6c60", shadow.size = 0.005, cex = cexd) 
textrect(c(cols[1], rows[1]), boxsize, 
            lab = "Combine  Silva \n and riboSeed 16S",
...
textrect(c(cols[3], rows[1]), boxsize, 
            ...
            box.col = "#d5b8af",rady=thisrady, radx=thisradx,
             shadow.col = NA, shadow.size = 0.005, cex = cexd) 
textrect(c(cols[5], rows[1]), boxsize, 
...
## Methods
```{r}
summary_data <- read.csv("summary_all", sep="\t", 
                         col.names = c("org", "status", "state", "message"))

ggplot(summary_data %>% filter(status == "FAIL"), aes(x=message)) + 
