...
```
# Introduction
This document contains all the of steps needed to reproduce the findings presented in this work, with the exception of the community analyses described in `DADA2_analyses.Rmd`; it was written with literate programming, so all code can be viewed in the source document at <https://github.com/femlab/focusdb_manuscript/blob/master/docs/>.

## Third-party software vesions

The conda environment used for the analyses is defined in  <https://github.com/femlab/focusdb_manuscript/blob/master/focusDB_env.txt>.
...
srac <- data.frame("org"=NA, "ncount"=NA, stringsAsFactors = F)
...
paste0(ngenerawith_sras, " of ", nunique_genera, " genra have availabe SRA data ")
...
- `--sge`: distribute assembly tasts as an SGE array job on the HPC
...
After execution of thhe pipeline, all of the references considered were recorded as follows:
```
while read genus ; do \
...
In the course of comparing the reassembled sequences to those for which reference genomes were available, we determined that the pipeline struggled in two cases: the *Treponema* genus, and one *Bacillus anthracis* SRA: SRR2155541.  We do not currently know the issue with the *Treponema* genus, but using blobtools we determined that SRR2155541 is contaminated with *Micrococcus luteus*:
...
In addition to those mentioned above with poor assemblies, we filtered out any SRAs with inconclusive domain-level agreement, where Kraken reports the top level Domain taxonomy with less than 70% aggreement. This primarily intended to remove datasets with poor host-DNA depletion.
...
ggsave( file="docs/figures/aggreement.png", device = "png", dpi = 300, width = 5, height = 5,
        ggplot(tax_summary, aes(x=domain_perc, fill=domain_perc>70)) + geom_histogram() + 
...
```{r tax-agreement-plot, echo=FALSE, fig.cap="Datasets where the average taxanomic assignment was less than 70 percent bacteria were removed, as these are likely contaminated with host DNA", out.width = '50%'}
...
- `Error`:
  ...
  - Unknown errors
- `QC Fail`:
  - Invaid library type (eg mate-paired reads)
...
build_aln_for_assembly <- function(db, assembly="GCA_000699725.1", trimf=10, trimr=10){
  ...
  consensus <- ConsensusSequence(trimmed[2],threshold = .999990, noConsensusChar="N", minInformation=1, includeTerminalGaps = F)
  # trimmed[length(trimmed) + 1] <- consensus
  # BrowseSeqs(trimmed, highlight = 1)
  ...
  if (length(columns_to_view) == 0){
    ...
    names(SNP_aln) <- names(trimmed)
    print("saving")
    BrowseSeqs(SNP_aln, highlight = 2, htmlFile = 
                 paste0("docs/figures/draft_alignments/", assembly, "_", sprintf("%04d", length(columns_to_view)), "_snps_", length(focus_subset_seqs), "_focusSeqs_", length(silva_subset_seqs), '_silvaSeqs.html'), openURL = F)
    # 
...
```{r assess_assembly_snps, cache=TRUE, eval=REPROCESS_RESULTS}
