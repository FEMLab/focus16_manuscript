...
p_summary_data_pre <- merge(
  ...
  select(org, globalmessage, message, state ) %>%  distinct()

p_summary_data <- merge(srac,  p_summary_data_pre, by.x="strain", by.y="org") %>% 
  mutate(
    pmessage = case_when(
      is.na(message) ~ "QC Fail",
      startsWith(message, "Reads were") ~ "QC Fail",
      startsWith(message, "Unable to process") ~ "QC Fail",
      message=="" ~ "16S Recovered", 
      message=="riboSeed unsuccessful" ~ "No 16S Recovered")) %>%
  arrange(pmessage) %>%
  group_by(strain) %>%
...
(p_outcomes <- ggplot(p_summary_data, 
                      aes(x=reorder(strain, desc(strain)), y=thisid, shape=pmessage,
...
silva = readRNAStringSet("~/Downloads/SILVA_132_SSURef_tax_silva.fasta")
for (organism in c("Bacteroides cellulosilyticus", "Bacteroides fragilis", "Bacteroides ovatus",
  ...
  uniqueness <- rbind(
    uniqueness, 
    data.frame("Organism"=organism, "Unique"=thistab[[1]],  "Duplicated"=thistab[[2]], "DB"="SILVA"))
  ...
  uniqueness <- rbind(
    uniqueness, 
    data.frame("Organism"=organism, "Unique"=thistab[[1]],  "Duplicated"=thistab[[2]], "DB"="focusDB + SILVA"))
  
}

(p_unique <- ggplot(uniqueness %>% gather(key="grp", value="val", -Organism, -DB ), 
       aes(x=Organism, y=val, fill=DB)) +
  coord_flip() +
  geom_bar(stat="identity", position="dodge") + facet_wrap(~grp) 
)

ggsave("docs/figures/2-unique.png", p_unique, device = "png", dpi = 300, width = 7, height = 5)
