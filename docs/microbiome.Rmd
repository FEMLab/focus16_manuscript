...
fg$cleangenus <- ifelse(grepl("\\d", fg$cleangenus),fg$Family, fg$cleangenus)
fg$cleangenus <- ifelse(is.na(fg$cleangenus),fg$Family, fg$cleangenus)
...
```{bash}
while read genus species; do 16db  -o ${genus}_${species} -g ${genus}_${species}_genomes -n "$genus $species" --cores 32 --memory 80 --n_references 50 --n_SRAs 20 --subassembler spades --threads 2  --maxdist .1  ; done < ./docs/microbiome_data/endo_species.txt
cat ./*/SUMMARY  > 2019-10-08-endo-results/endo_summay_all
cp ./*/*ribo16s.fasta ./2019-10-08-endo-results/
```

```{r}
endo_data <- read.csv(
  "./results/2019-10-08-endo-results/endo_summary_all.tab",
...
for (path in Sys.glob("./results/2019-10-08-endo-results/*_ribo16s.fasta")){
...
taxa_silva_both <- assignSpecies(taxa, "./docs/microbiome_data/combined.fasta")
