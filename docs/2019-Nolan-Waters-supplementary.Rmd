...
(p_outcomes <- ggplot(p_summary_data, 
    ...
    labs("SRAs Proccessed by focusDB",
         y="Number of SRAs", x="Organism", color="Run Status", fill="Run Status", shape="SRA status")  +
...
(p_unique <- ggplot(
  ...
  aes(x=Organism, y=val, alpha=grp, group=grp, fill=DB)) +
    ...
    labs(x="Number of Sequence", y="Organism", alpha="", fill="Database") +
    theme(axis.text=element_text(size=13)) +
...
right_join(B, A, by=c("strain"="Organism")) %>% 
  ...
  select(Genus, Species, All_SRAs, Good_SRAs, focus, silv, combined) %>% View()
```
...
silva_sras <- read.csv("./docs/accessions", sep="\t", header = F, col.names = names,stringsAsFactors = F)
#(silva_sras$org <- gsub("(\\W*)\\s(\\W)[$| ]*", "\\1", silva_sras$ScientificName))
(p_summary <- silva_sras  %>% group_by(ScientificName,  assembly) %>% 
    select(ScientificName, assembly, silva) %>% distinct() %>%  summarize(n=n()))
# left_join(p_summary%>%filter(n!=1), silva_sras) %>% select(ScientificName, Run) %>% distinct() %>%View()
# left_join(p_summary%>%filter(n==1), silva_sras) %>% select(ScientificName, Run)
ovatus <- p_summary[grepl("ovatus", p_summary$ScientificName),  ]
fragilis <- p_summary[grepl("fragilis", p_summary$ScientificName),  ]
```
Here,  looking at _B. ovatus_, we can see  that GCF_000699725.1 has 4 , so we dont need to reassemble those: rrndb shows that we can expect around  5 rDNAs for B ovatus.  For starters though, lets assess how we assembledthe GCF_000699725.1

```{r eval=FALSE}
build_aln_for_assembly <- function(assembly="GCF_000699725.1"){
  this_silva <- unique(silva_sras[grepl(assembly, silva_sras$assembly), "silva" ])
  # paste tofgether to make a query string with ors
  this_silva_query <- gsub("(.*)\\|$", "\\1", paste0(gsub("(.*?)\\..*", "\\1", this_silva), collapse = "", sep="|"))
  this_focus <- unique(silva_sras[grepl("GCF_000699725.1", silva_sras$assembly), "Run" ])
...
GCF_000699725 <- build_aln_for_assembly(assembly="GCF_000699725.1")
GCF_000699665 <- build_aln_for_assembly(assembly="GCF_000699665.1")
# BrowseSeqs(GCF_000699725)
# BrowseSeqs(GCF_000699665)
```
Now we can compare.  

```{r eval=FALSE}
fragilis[1, "assembly"]
GCA_001058745 <- build_aln_for_assembly(assembly=fragilis[1, "assembly"])
BrowseSeqs(GCA_001058745)
```
