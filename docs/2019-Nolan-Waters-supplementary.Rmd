...
for (organism in c("Bacteroides cellulosilyticus", "Bacteroides fragilis", "Bacteroides ovatus",
  ...
  for (header in names(focus_subset)){
    ...
    if (!grepl("extracted", header)){
      ...
      isdraft <-  nchar(gsub("(\\D*).*", "\\1", header)) > 3
      # skip if not wgs
      if (! isdraft) {next}
      id = gsub("(.*?)\\.(.*)", "\\1", header )
...
ovautus <- p_summary[grepl("ovatus", p_summary$ScientificName),  ]
```
...
```{r}
ovatus_silva <- unique(silva_sras[grepl("GCF_000699725.1", silva_sras$assembly), "silva" ])
# paste tofgether to make a query string with ors
ovatus_silva_query <- gsub("(.*)\\|$", "\\1", paste0(gsub("(.*?)\\..*", "\\1", ovatus_silva), collapse = "", sep="|"))
ovatus_focus <- unique(silva_sras[grepl("GCF_000699725.1", silva_sras$assembly), "Run" ])
ovatus_focus_query <- gsub("(.*)\\|$", "\\1", paste0(gsub("_\\d+", "",ovatus_focus), collapse = "", sep="|"))

silva_seqs <- silva[grepl(ovatus_silva_query,  names(silva))]
focus_seqs <- all_focus[grepl(ovatus_focus_query, names(all_focus))]
setdiff(focus_seqs, DNAStringSet(silva_seqs))
pairwiseAlignment(pattern = DNAStringSet(silva_seqs), subject = focus_seqs)
# keep in mind  comparing an RNAString object with a DNAString object,
