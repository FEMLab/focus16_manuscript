...
bal_data <- read.csv("results/2019-10-03-focusdb-balanced/summary_balanced_all.tab", sep="\t", header=F,
                         col.names = c("org", "status", "state", "messag"), stringsAsFactors = F)
bal_data$mock <- "Balanced"
hmp_data <- read.csv("results/2019-10-03-hmp-focusdb/summary_hmp_all.tab", sep="\t", header=F,
                         col.names = c("org", "status", "state", "messag"), stringsAsFactors = F)
hmp_data$mock <- "HMP"
ex_data <- read.csv("results/2019-09-26-focusdb-results/summary_extremes_all", sep="\t", header=F,
                         col.names = c("org", "status", "state", "messag"), stringsAsFactors = F)
...
(p_hmp <- plot_mock(df=p_summary_data%>% filter(mock == "HMP")))
(p_ex <- plot_mock(df=p_summary_data%>% filter(mock == "Extremes")))
(p_bal <- plot_mock(df=p_summary_data%>% filter(mock == "Balanced")))
