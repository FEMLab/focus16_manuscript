...
#(draft_silva_sras$org <- gsub("(\\W*)\\s(\\W)[$| ]*", "\\1", draft_silva_sras$ScientificName))
...
(p_summary_cg    <-    cg_silva_sras  %>% group_by(ScientificName,  assembly) %>% 
    select(ScientificName, assembly, silva) %>% distinct() %>%  summarize(n=n()))
# left_join(p_summary_draft%>%filter(n!=1), draft_silva_sras) %>% select(ScientificName, Run) %>% distinct() %>%View()
# left_join(p_summary_draft%>%filter(n==1), draft_silva_sras) %>% select(ScientificName, Run)
...
GCF_000699725 <- build_aln_for_assembly(db=draft_silva_sras, assembly="GCF_000699725.1")
GCF_000699665 <- build_aln_for_assembly(db=draft_silva_sras, assembly="GCF_000699665.1")
