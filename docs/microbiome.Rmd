...
The first time we ran the analysis (before having the focusDB results), here is what we did to generate the species files that were used when running focusDB:

```{r, eval=FALSE}
endo_taxa <- assignTaxonomy(endo_seqtab.nochim, "~/Downloads/silva_nr_v132_train_set.fa.gz", multithread=TRUE)
...
# here, if the genus level annotation is something like "group" or "clade" or otherwise unhelpful, we tidy it up, or use the "family" level annotation if needed.  These get fixed later when making the combined list for the actual run.
...
write.table(together[together$silva!=together$focuDB, ], file = "STABLE_different_assignment.tab", row.names = F, sep="\t")
