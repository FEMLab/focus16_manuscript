...
#          )) %>% BrowseSeqs()                  
  ...
  these_results = data.frame("set"=character(), "focusDB"=character(),       "ref_gcn"=numeric(), "ref_gcn_unique"=numeric(), "silva"=character(), "pkb"=numeric(), stringsAsFactors = F)
...
(p_snps <- ggplot(snp_results %>% filter(!is.na(pkb)), aes(x=set )) + 
    ...
    geom_point(aes(y=pkb, color=assembly), alpha=.6,position=position_jitter(width = .2, height = .002, seed = 123)) +  
