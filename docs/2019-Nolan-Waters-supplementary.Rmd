...
- `--n_SRAs 50`: limits reassembly to 50 random SRA read sets per genus
- `--run_de_novo_control`: ensures riboSeed run a control *de novo* assembly with SPAdes.  This is used for comparisons of the *de novo* assembly to the *de fere novo* assemblies.
...
- `--use_available`: use any reads downloaded  while previously processing an SRA
...
do
    ...
    counter=$((counter + 1))
done < docs/datasets/combined_genuses.txt
```
...
while read genus; do echo $genus; cp ./${genus}_genus/${genus}_fast_ribo16s.fasta ./2019-12-19-results/;  cp ./${genus}_genus/${genus}_full_ribo16s.fasta ./2019-12-19-results/ ; cp ./${genus}_genus/focusDB.log ./2019-12-19-results/${genus}.log ; cp ./${genus}_genus/sequence_summary.tab  ./2019-12-19-results/${genus}_seq_summary.tab; done < docs/datasets/combined_genuses.txt
...
echo -e "${genus}\t${j}" >> docs/reference_genomes.tab ; \
done ;done < docs/datasets/combined_genuses.txt
```
...
```{r reference-datasets, echo=FALSE, fig.cap="A median of 9 references were available for each of the genera considered. The number of references per genera was capped at 200.  In practice, this only effected the Bordetella genus.  Data were plotted with ggbeeswarm using the quasirandom layout to show the distribution.", out.width = '80%'}
...
In the course of comparing the reassembled sequences to those for which reference genomes were available, we determined that the pipeline struggled in two cases: the *Treponema* genus, and one *Bacillus anthracis* SRA: SRR2155541.  We do not currently know the issue with the *Treponema* genus, but using blobtools we determined that SRR2155541 is likely contaminated with *Micrococcus luteus* (Figure \@ref(fig:blob-bacillus)).

GCA_000755185.1

```{bash blobforSRR2155541, eval=FALSE, echo=TRUE}
# we downloaded GCF_000008165.1_ASM816v1_genomic.fna as a reference
docker run --memory 16G --rm  -v $PWD:/home/ nickp60/ezblobtools  -r /home/GCF_000008165.1_ASM816v1_genomic.fna \
  -A /home/results/2019-12-19-results/Bacillus_genus/SRR2155541/results/riboSeed/seed/final_de_novo_assembly/contigs.fasta   \
  -d ref_prok_rep_genomes -o /home/GitHub/focusdb/docs/blob_SRR2155541/ -t 2 -m 16
  
docker run --memory 16G --rm  -v $PWD:/home/ nickp60/ezblobtools   -A /home/results/2019-12-19-results/Corynebacterium_genus/SRR2889779/results/riboSeed/seed/final_de_novo_assembly/contigs.fasta     -d ref_prok_rep_genomes -o /home/blob_SRR2889779/ -t 2 -m 16
```
...
```{r tax-agreement-plot, echo=FALSE, fig.cap="Datasets where Kraken's taxanomic assignment of preliminary assembly contigs showed less  less than 70 percent agreement  were removed, as these are likely contaminated with host DNA", out.width = '80%'}
knitr::include_graphics("figures/aggreement.png")
```

Figure \@ref(fig:tax-agreement-plot) shows a histogram of the domain-level agreement. We parse the outputs from Kraken2's taxonomic assignment, and flag any with a low (<70) percent of domain-assigned sequences, as this could indicate that an SRA is contaminated.  Some (such as Treponema SRAs 	SRR3571775 and SRR3584844) have $\approx$ 60% human DNA, indicating incomplete removal of host DNA, and as such we drop them, losing 9 SRAs.  However, it should be noted that BLASTing contigs from these assemblies appear to show bacterial DNA -- the Kraken2 database itself may have contamination where Treponema kmers are labelled as human.  Either way, these (and any other under the 70% domain-level assignment threshold) are dropped.


# Results
## Assessing per-SRA outcomes of the pipeline

The outcome of each SRA was determined after running the pipeline. For the purpose of this figure, the following messages were combined:
...
p_summary_data <- right_join(srac,  p_summary_data_pre, by=c("org")) %>% 
  mutate(
    ...
    pmessage = case_when(
      ...
      messag=="" ~ "16S rRNA Recovered", 
      messag=="riboSeed unsuccessful" ~ "No 16S rRNA Recovered")) 
...
## Re-extracting 16S rRNAs fro SILVA
In order to 

```{bash eval=FALSE}
seqkit split2 ~/Downloads/SILVA_132_SSURef_tax_silva.fasta.gz  --out-file silva132_ --by-part 1000
for i in $(ls /Users/nick/Downloads/SILVA_132_SSURef_tax_silva.fasta.gz.split/SILVA_132_SSURef_tax_silva.part_*.gz); do echo $i ; zcat < $i | barrnap -t 8 --outseq ${i}.just16s.fasta --reject .8 ; done
cat  /Users/nick/Downloads/SILVA_132_SSURef_tax_silva.fasta.gz.split/SILVA_132_SSURef_tax_silva.part_*.gz.just16s.fasta > ~/Downloads/SILVA_trimmed.fasta 

zcat < /Users/nick/Downloads/SILVA_132_SSURef_tax_silva.fasta.gz | grep ">" | wc -l 
# 798129
cat ~/Downloads/SILVA_trimmed.fasta | grep ">" | wc -l 
# 2090668
```


```{r}
silva_rrnas <- Biostrings::readRNAStringSet("~/Downloads/SILVA_trimmed.fasta")
silva_rrnas <- silva_rrnas[grepl("16S_", names(silva_rrnas))]
names(silva_rrnas) <- gsub("16S_rRNA::", "",  names(silva_rrnas), fixed = TRUE)

```


## Extracting 16S rRNA from de novo assemblies
For the comparison of error rates between the *de fere novo* assembly and *de novo* assembly, `make_silva_style_db_from_contigs.py` was used  to identify 16S rRNA regions using Barrnap and extract those regions from the  *de novo* contigs.  
...
We removed any from the list as identified above in section "Identifying poor taxonomic assignments".
```{bash, eval=FALSE}
...
We read in the SILVA database, and parsed the headers using the prefix mapping  found at <https://www.ncbi.nlm.nih.gov/Sequin/acc.html>. Each header for the SILVA database consists of `>ACCESSION.start.end name`. This revealed the origin of the individual sequences in the database.
...
length(silva)
 length(unique(silva))
 length(datasets[["fast"]][["set"]])
 #  5647
 length(unique(datasets[["fast"]][["set"]]))
 # 2823
length(setdiff(unique(datasets[["fast"]][["set"]]), unique(silva)))
length(intersect(unique(datasets[["fast"]][["set"]]), unique(silva)))
length(intersect(unique(datasets[["fast"]][["set"]]), unique(silva_rrnas)))
```
...
build_aln_for_assembly <- function(db, assembly="GCA_000699725.1", trimf=10, trimr=10){
  ...
  if (length(unique_focus_subset_seqs) == 0) {
    warning(paste("No matches for ",this_focus_query,"(", assembly, ")  in focus16 results"))
    return (list(NA, NA, NA, NA, NA, NA))
  }
...
build_pairwise_network <- function(db, assembly="GCA_000699725.1"){
  ...
  if (length(this_silva) == 0) {
    warning(paste("No matches ", assembly, ";  could be long reads?"))
    return (list(NA, NA, NA, NA, NA,NA, NA, NA, NA, NA,  NA))
  }
  # paste tofgether to make a query string with or's; this is because for assemblies, 
  # # different contigs will have different sequence names, but a single assembly accession
  #this_silva_query <- gsub("(.*)\\|$", "\\1", paste0(unique(gsub("(.*?)\\..*", "\\1", this_silva)), collapse = "", sep="|"))
  #silva_subset_seqs <-  silva[grepl(this_silva_query,  names(silva))]
#      old_silva_subset_seqs <-  silva[grepl("CP008729|CP008730|CP008728",  names(silva))]
#      old_silva_subset_seqs <-  silva[grepl(this_silva_query,  names(silva))]
# DECIPHER::AlignSeqs(
#   append(append(DNAStringSet(silva_subset_seqs),
#                DNAStringSet(old_silva_subset_seqs)),
#                DNAStringSet(focus_subset_seqs),
#          )) %>% BrowseSeqs()                  
  ...
  # identify all sequences associated with this assembly accesssion
  # this now uses the assembly-accession-based name rather than the chromosome-based-name
  silva_subset_seqs <- rrnas[grepl(assembly,  names(rrnas))]
  # idenify the sequences from focusDB fast, align them, keep track of best alignments
  this_row <- NULL
  this_aln <- NULL
  these_results = data.frame(
    "set"=character(), "focusDB"=character(),       
    "ref_gcn"=numeric(), "ref_gcn_unique"=numeric(), "silva"=character(),
    "pkb"=numeric(),
    "pkb_v13" = numeric(),
    "pkb_v34" = numeric(),
    stringsAsFactors = F)
  for (thisset in c("fast", "full", "denovo")){
   print(paste0(" set: ", thisset))
   focus_subset_seqs <-  datasets[[thisset]][["set"]][grepl(this_focus_query, names( datasets[[thisset]][["set"]]))]
    ...
    for (i in 1:length(focus_subset_seqs)){
      ...
      overall_error_rate = 1000
      
      for (j in 1:length(silva_subset_seqs)){
        ...
        # scoreOnly=FALSE, type="overlap") %>% writePairwiseAlignments("tmp.pair")
        # DECIPHER::AlignSeqs(append(DNAStringSet(focus_subset_seqs[i]), DNAStringSet(silva_subset_seqs[j]))) %>% BrowseSeqs(highlight=0)
        ...
        # here we switch to comparing error rates, not raw score
        alignment_length <- width(alignedSubject(aln))
        errors_pkb <- ((alignment_length - aln@score ) * 1000) / alignment_length
        if  (errors_pkb <  overall_error_rate){
          this_row <-  data.frame(
            "set"=thisset,
            ...
            "pkb_v13" = NA,
            "pkb_v34" = NA,
            "ref_gcn"=length(silva_subset_seqs),
            "ref_gcn_unique"=length(unique(silva_subset_seqs)),
            stringsAsFactors = F)
          this_aln <- aln
          if (verbose) writePairwiseAlignments(this_aln, file=file.path("cg_alignments", paste0(thisset, "_",  gsub("(.*?) .*", "\\1", names(focus_subset_seqs[i])))))
        }
      }
      # we only add the top hit for each focusDB sequence
      # if no good hits exist, thisrow wont exist
      if (exists("this_row")){
                  # run the region-specific alignments
          # Turn degenerate primers into DNAsstringsets
          #illumina/https://pdfs.semanticscholar.org/ecb7/a216272a400d86b7a3e3bee35f721ab127a1.pdf
          # https://www.hmpdacc.org/hmp/doc/16S_Sequencing_SOP_4.2.2.pdf
          regions=list(
            pkb_v13=list("F"="AGMGTTYGATYMYGGCTCAG", 
                         "R"="ATTACCGCGGCTGCTGG",
                         "len"=507,
                         "anneal_temp"=56), # old school AGAGTTTGATCCTGGCTCAG
            pkb_v34=list("F"="CTACGGGNGGCWGCAG",
                         "R" ="GACTACHVGGGTATCTAATCC", 
                         "anneal_temp"=55,
                         "len"=444)
          )
          for (reg_i in 1:length(regions)){
            if (verbose) print(paste0("  - getting amplicon error rates for",this_row$focusDB, " within ", names(regions)[reg_i]))
            #PCR 
          # trim_focus <- DECIPHER::AmplifyDNA(DNAStringSet(focus_subset_seqs[i]), maxProductSize = 600,  annealingTemp = reg$anneal_temp, P = 4e-7, primers =c(reg$F, reg$R), includePrimers=TRUE)
          # f_pct <- as.numeric(gsub("(.*)\\%.*", "\\1",  names(trim_focus) ))[1]
          # trim_silva <- DECIPHER::AmplifyDNA(DNAStringSet(silva_subset_seqs[i]), maxProductSize = 600,  annealingTemp = reg$anneal_temp, P = 4e-7, primers =c(reg$F, reg$R), includePrimers=TRUE)
          # s_pct <- as.numeric(gsub("(.*)\\%.*", "\\1",  names(trim_focus) ))[1]
          # 
          # This is a bit of a mess, but we use Biostring comparestrings to get a consensus 
          # with ambiguity, and we replace all gaps/mismatches with N's so we can
          # use DECIPHER's AmplifyDNA
          # Then, we count all the Ns in the resulting amplicon
          trimmed_aln <- DECIPHER::AmplifyDNA(
            DNAStringSet(gsub("[^ATGC]", "N", compareStrings(this_aln))), 
            maxProductSize = 600,  annealingTemp =  regions[[reg_i]]$anneal_temp,
            P = 4e-7, primers =c(regions[[reg_i]]$F, regions[[reg_i]]$R), includePrimers=TRUE)
          if (length(trimmed_aln) == 0){
            warning("PCR failure")
            amplicon_err_rate_ = NA
          } else {
          alignment_pct_ <- as.numeric(gsub("(.*)\\%.*", "\\1",  names(trimmed_aln)))[1]
          n_errors_ <- nchar(gsub("[^N]", "", trimmed_aln[[1]]))
          amplicon_len_ <- nchar(trimmed_aln[[1]])
          amplicon_err_rate_= n_errors_ * 1000 /amplicon_len_ 
          
          if(alignment_pct_ < 90){
            warning("poor quality PCR")
            amplicon_err_rate_ = NA
           }
          }
          this_row[, names(regions)[reg_i]] = amplicon_err_rate_
          }
            these_results <- rbind(
              these_results,
              this_row)
...
writeLines(unique(only_illumina_assembly_summary$Assembly.Accession), con = "./complete_genome_assembly_accessions.txt")
```
...
Note (2022-03-12): this was insufficient in cases such as Burkholderia multivorans posessing multiple chromosomes. use Entrez Batch to download the 61 assemblies in `complete_genome_assembly_accessions.txt` to ./docs/genome_assemblies_genome_fasta/ ( it will have a dated subdirectory)

```{bash eval=FALSE}
cat  docs/complete_genome_assembly_accessions.txt | while read x ; do echo $x ; zcat < ./docs/genome_assemblies_genome_fasta/ncbi-genomes-2022-03-12/$x* | barrnap - --outseq ${x}rrna.fasta ; done
cat ./*rrna.fasta > complete_genomes_rrnas.fasta
```


```{r,  eval=REPROCESS_RESULTS}
rrnas <- map(unique(only_illumina_assembly_summary$Assembly.Accession), .f=function(x){
seqs <- Biostrings::readDNAStringSet(paste0("../", x, "rrna.fasta"))
seqs <- seqs[grepl("16S_", names(seqs))]
names(seqs) <- paste0(x, "..", names(seqs))
seqs
})
rrnas <- do.call(c, rrnas)
```

```{r assess_cg_snps, cache=TRUE, eval=REPROCESS_RESULTS}
# dest_dir <- file.path("docs", "ref_msas")
# dir.create( dest_dir)
dir.create("cg_alignments")
...
#for (i in 1:50){
  ...
  if (assembly %in% snp_results$assembly){
    ...
    print(paste("took", Sys.time()-t0, "for last assembly; processing", counter + 1, "of", nassemb))
    t0 <- Sys.time()
    snp_results <- rbind(
      snp_results,
...
#how many total seqs are we anticipating?  Not exactly the sum of all the 16S's in the assemblies cuase some SRAs contributed to the same complete genome (such as SRR4124911 and SRR4124912)

(target_16S_count <- snp_results %>% mutate(sra = gsub("^(.*?)_.*", "\\1", focusDB)) %>% select(sra, assembly, ref_gcn) %>% distinct() %>% pull(ref_gcn) %>% sum())
# remove NAs
(p_snps <- ggplot(snp_results %>% filter(!is.na(pkb)), aes(x=set )) + 
    mytheme +
    #geom_bar(aes(y=..count../10), fill="grey80")+ 
    geom_label(stat='count', aes(y=(..count../10) +15, label=paste("n = ", ..count.., " of ", target_16S_count, "\ntrue sequences" )), size=3,label.size=0 , color="grey60")+ 
    geom_label(data=snp_results %>% filter(pkb==0), stat='count', aes(y=(..count.. * 0) -1, label=paste(..count.., " with 0 errors")), size=3,label.size=0 , color="grey60")+ 
    geom_boxplot(aes(y=pkb, color=set), color="grey10", fill=NA, outlier.shape = NA, outlier.colour = "grey") + 
    geom_point(aes(y=pkb, color=assembly), alpha=.7,position=position_jitter(width = .2, height = .002, seed = 123)) +  
   ggrepel::geom_text_repel(aes( color=assembly, y=pkb, label=ifelse(pkb > 6, assembly, "")), #color="black",
                            alpha=.8,position=position_jitter(width = .2, height = .002, seed = 123), size=2, max.overlaps = 15) +  
...
regions_results <- snp_results %>%
  filter(!is.na(pkb)) %>% 
  pivot_longer(cols=c(pkb, pkb_v13, pkb_v34), names_to="region", values_to="pkb") %>%
  mutate(clean_region =case_when(
    region == "pkb_v13" ~ "V1-V3\n(507bp)",
    region == "pkb" ~ "Full-Length\n(~1520bp)",
    TRUE ~ "V3-V4\n(444)"),
    label = ifelse(pkb > 10, assembly, "")) %>%
  group_by(region, assembly) %>% 
  arrange(desc(pkb)) %>%
  mutate(label = ifelse(row_number() == 1, label, ""))

(p_snps_sup <- ggplot(regions_results,  aes(x=set )) + 
    mytheme +
    geom_label(data=regions_results %>% filter(region == "pkb"), stat='count', aes(y=(..count../10) +15, label=paste("n = ", ..count.., "/", target_16S_count )), size=3,label.size=0 , color="grey60")+ 
    # geom_label(stat='count', aes(y=(..count../10) +15, label=paste("n = ", ..count.., " of ", target_16S_count, "\ntrue sequences" )), size=3,label.size=0 , color="grey60")+ 
    geom_label(data=regions_results %>% filter(pkb==0), stat='count', aes(y=(..count.. * 0) -1.5, label=paste(..count.., "")), size=3,label.size=0 , color="grey60")+ 
    geom_boxplot(aes(y=pkb, color=set), color="grey10", fill=NA, outlier.shape = NA, outlier.colour = "grey") + 
    geom_point(aes(y=pkb, color=assembly), alpha=.7,position=position_jitter(width = .2, height = .002, seed = 123)) +  
   ggrepel::geom_text_repel(aes( color=assembly, y=pkb, label=label), #color="black",
                            alpha=.8,position=position_jitter(width = .2, height = .002, seed = 123), size=2, max.overlaps = 15) +  
    scale_color_discrete(guide="none") +
    facet_wrap(~clean_region) + 
    labs(#title="Accuracy of 16S rRNA sequences by assembly method ", 
         #subtitle="gray bars represent the number of sequences recovered",
         x= "focus16  Mode",
         color="Assembly",
...
(p_snps2 <- ggplot(snp_results %>% filter(!is.na(pkb)), aes(x=set )) + 
    mytheme +
    #geom_bar(aes(y=..count../10), fill="grey80")+ 
...
(p_snps_hist_regions <- ggplot(
  regions_results %>% filter(!is.na(pkb)) %>%filter(set=="fast")%>%
    group_by(clean_region, region) %>%
    mutate(bin=cut_width(pkb, width = 1)) %>%
    select(-pkb) %>%
    group_by(clean_region, region, bin) %>% 
    summarize(y=n()) %>% 
    mutate(x=as.numeric(gsub(".*,(\\d*\\.\\d).", "\\1", bin)) - .5), aes(x=x, y=y, fill=clean_region)) + 
    mytheme +
    geom_bar(stat="identity", position=position_dodge2(width=.95, preserve = "single")) +
    scale_fill_manual(values=wesanderson::wes_palettes$IsleofDogs1[1:3]) +
    scale_x_continuous(breaks = seq(0, max(regions_results$pkb, na.rm =TRUE), 5)) + 
...
(p_snps_hist2 <- ggplot(snp_results %>% filter(!is.na(pkb)) %>%filter(set=="fast")%>%
                        ...
                        aes(x=bin, y=y, color=bin==0)) + 
    mytheme +
    #geom_bar(stat="identity", width = .8, positiion="dodge") +
    scale_color_manual(guide="none", values=wesanderson::wes_palettes$IsleofDogs1[4:5]) +
    geom_point(aes(y=cumuy),  size=2) +
    ...
    scale_y_continuous(limits = c(0, 100),breaks=seq(0, 100, length.out=6)) 
)
(p_snps_hist2_regions <- ggplot(regions_results %>% filter(!is.na(pkb)) %>%filter(set=="fast")%>%
                         group_by(clean_region, region, pkb) %>%
                         summarize(bin=unique(ceiling(pkb)), 
                                y=n()) %>% mutate(
                                cumuy=cumsum(y)/sum(y) * 100) ,#%>% transform(y=y/sum(y) * 100), 
                        aes(x=bin, y=y, color=bin==0)) + 
    ...
    #geom_hline(yintercept = 99, color="gray50", linetype="dashed") + 
    #annotate(geom="text", x = 3, y=95, label="99%", color="gray50") + 
    facet_wrap(~clean_region, ncol=3) +
    scale_y_continuous(breaks=seq(0, 100, length.out=5), expand = c(0, 0)) +
    labs(
      x="Errors/Kbp", y="Percent of Sequences",
      #subtitle="Cumulative distribution "
    ) +
    scale_y_continuous(limits = c(25, 100),breaks=seq(0, 100, length.out=6)) 
)
...
CCCCC
CCCCC"
new_fig2 <- p_snps_sup +  p_snps_hist2_regions + #p_snps_hist_regions+coord_cartesian(xlim=c(0, 10)) +
  p_snps_hist_regions +  #scale_y_log10(expand=c(0, 0)) 
  plot_annotation(tag_levels = "A")+  plot_layout(design = des, guides="collect") &
  theme(legend.position = 'bottom',
        legend.direction = 'horizontal') 
ggsave("../docs/figures/new2-accuracy.png", new_fig2, device = "png", dpi = 300, width = 12, height = 8, scale=1)
ggsave("../docs/figures/S5-set-accuracy.png", p_snps, device = "png", dpi = 300, width = 7, height = 5)
ggsave("../docs/figures/S5b-set-accuracy-regions.png", p_snps_sup, device = "png", dpi = 300, width = 8, height = 6)
...
summarize(bin=unique(ceiling(snps)), 
                                y=n()) 
draft_alns_sum[nrow(draft_alns_sum) + 1 ,] <- tibble(snps=0, bin=0, y=nassemblies-nrow(draft_alns))
